...
void CCNF_patch_expert::ResponseOpenBlas(const cv::Mat_<float> &area_of_interest, cv::Mat_<float> &response, cv::Mat_<float>& im2col_prealloc)
{
	...
	//cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, normalized_input.cols, weight_matrix.rows, weight_matrix.cols, alpha1, (float*)normalized_input.data, normalized_input.cols, (float*)weight_matrix.data, weight_matrix.cols, beta1, (float*)neuron_resp_full.data, normalized_input.cols);\
	...
	//cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, resp_vec_f.cols, Sigmas[s_to_use].rows, Sigmas[s_to_use].cols, alpha1, (float*)resp_vec_f.data, resp_vec_f.cols, (float*)Sigmas[s_to_use].data, Sigmas[s_to_use].cols, beta1, (float*)out.data, resp_vec_f.cols);
