...
public:
	...
	//init
	void init(string root, string fname);
	void  initRespond(FaceModelParameters& params);
	...
	void Read(string root, string name);
	
private:

	// Helper reading function
	bool Read_CLNF(string root, string clnf_location);
