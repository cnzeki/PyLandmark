...
CLNF::CLNF(string root, string fname)
{
	// A successful read wil set this to true
	loaded_successfully = false;

	this->Read(root, fname);		
}
...
CLNF::CLNF(const CLNF& other): pdm(other.pdm), params_local(other.params_local.clone()), params_global(other.params_global), detected_landmarks(other.detected_landmarks.clone()),
	landmark_likelihoods(other.landmark_likelihoods.clone()), patch_experts(other.patch_experts), landmark_validator(other.landmark_validator), hierarchical_mapping(other.hierarchical_mapping), hierarchical_models(other.hierarchical_models), hierarchical_model_names(other.hierarchical_model_names),
	hierarchical_params(other.hierarchical_params), eye_model(other.eye_model), preference_det(other.preference_det), loaded_successfully(other.loaded_successfully)
{
...
void CLNF::init(string root, string fname)
{
	// A successful read wil set this to true
	loaded_successfully = false;
...
bool CLNF::Read_CLNF(string root, string clnf_location)
{
	// Location of modules
	string main_location = root + clnf_location;
	...
	{ 
		...
		getline(lineStream, sub_location);

		if (sub_location.size() > 0)
			sub_location.erase(sub_location.begin()); // remove the first space
				
		// remove carriage return at the end for compatibility with unix systems
		if (sub_location.size() > 0 && sub_location.at(sub_location.size() - 1) == '\r')
		{
			sub_location = sub_location.substr(0, sub_location.size() - 1);
		}

		// append the lovstion to root location (boost syntax)
		//location = (root / location).string();
		string location = root + sub_location;
...
void CLNF::Read(string root, string main_location)
{

	cout << "Reading the landmark detector/tracker from: " << main_location << endl;
	
	string  path = root + main_location;
	ifstream locations(path.c_str(), ios_base::in);
	if(!locations.is_open())
	{
	...
	{ 
		...
		{ 
			...
			bool read_success = Read_CLNF(root, sub_location);
		...
		else if(module.compare("LandmarkDetector_part") == 0)
		{
			...
			///string  part_location = root - location;
			CLNF part_model(root, sub_location);
...
void  CLNF::initRespond(FaceModelParameters& params)
{
	params.window_sizes_current;
	...
	for (int scale = 0; scale < num_scales; scale++)
	{
		...
		if (params.window_sizes_current[scale] == 0)
			continue;

		int window_size = params.window_sizes_current[scale];

		patch_experts.init(pdm, window_size, scale);
	}
}
