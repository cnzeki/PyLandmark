...
bool DetectLandmarksInImageMultiHypBasic(const cv::Mat_<uchar> &grayscale_image, vector<cv::Vec3d> rotation_hypotheses, const cv::Rect_<double> bounding_box, CLNF& clnf_model, FaceModelParameters& params)
{
	...
	float best_detection_certainty = 0;
	...
	//printf("best shape:  pitch: %f, yaw: %f, roll: %f\n", rotation_hypotheses[best_shape_id][0], rotation_hypotheses[best_shape_id][1], rotation_hypotheses[best_shape_id][2]);
	return best_success;
...
bool LandmarkDetector::DetectLandmarksInImage(const cv::Mat &rgb_image, const cv::Rect_<double> bounding_box, CLNF& clnf_model, FaceModelParameters& params, cv::Mat &grayscale_image)
{
	...
	if(params.multi_view)
	{
		...
		// It is possible to add other orientation hypotheses easilly by just pushing to this vector  pitch, yaw, roll
		//rotation_hypotheses.push_back(cv::Vec3d(0.5236, 0.0, 0.0));
...
// This is the one where the actual work gets done, other DetectLandmarksInImage calls lead to this one
bool LandmarkDetector::DetectLandmarksInImageUseInitOri(const cv::Mat &rgb_image, cv::Rect_<double> bounding_box, CLNF& clnf_model, FaceModelParameters& params, vector<cv::Vec3d>& rotation_hypothese)
{
	int x1 = std::max(0, int(bounding_box.x - bounding_box.width / 4));
	int y1 = std::max(0, int(bounding_box.y - bounding_box.height / 4));
	int x2 = std::min(rgb_image.cols - 1, int(bounding_box.x + bounding_box.width + bounding_box.width / 4));
	int y2 = std::min(rgb_image.rows - 1, int(bounding_box.y + bounding_box.height + bounding_box.height / 4));
	cv::Rect  faceBoundingBox = cv::Rect(x1, y1, x2 - x1, y2 - y1);

	cv::Mat grayscale_image = cv::Mat::zeros(cv::Size(faceBoundingBox.width, faceBoundingBox.height), CV_8UC1);
	if (rgb_image.channels() == 3)
	{
		cv::cvtColor(rgb_image(faceBoundingBox), grayscale_image, cv::COLOR_BGR2GRAY);
	}
	else
	{
		rgb_image(faceBoundingBox).copyTo(grayscale_image);
	}
	
	//cv::imshow("gray", grayscale_image);

	bounding_box.x = bounding_box.x - x1;
	bounding_box.y = bounding_box.y - y1;

	bool  success = DetectLandmarksInImageMultiHypBasic(grayscale_image, rotation_hypothese, bounding_box, clnf_model, params);

	int n = clnf_model.detected_landmarks.rows / 2;
	for (int l = 0; l < n; l++) {
		clnf_model.detected_landmarks.at<float>(l) = clnf_model.detected_landmarks.at<float>(l) +x1;
		clnf_model.detected_landmarks.at<float>(l+n) = clnf_model.detected_landmarks.at<float>(l+n) +y1;
	}
